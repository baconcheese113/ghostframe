...
async def start_analysis(request: AnalysisRequest) -> AnalysisResponse:
    ...
    try:
        ...
        steps.append(StepResult(name="MAF / FASTA", status="not_implemented",
                                detail="FASTA parsing not yet implemented"))
        return AnalysisResponse(job_id=job_id, status="partial", steps=steps,
                                error="FASTA parsing not yet implemented")
    except Exception as e:
        steps.append(StepResult(name="MAF / FASTA", status="error", detail=str(e)))
        return AnalysisResponse(job_id=job_id, status="error", steps=steps,
